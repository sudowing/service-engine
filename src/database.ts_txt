...
export const Database = class implements ts.IDatabase {
  ...
  genCoreTableQuery({
    userQuery: { table, where, filters, ranges, geoFilters, subquery },
    tableOptions,
    ...
    if (filters) {
      for (const { operation, field, value } of filters) {
        ...
        if (typeof value === "string" && value === "_NULL") {
          ...
          else if (operation === cnst.OP_NE_STRING) query.whereNotNull(field);
          else
            throw new Error(
  ...
  queryTableRecords({
    ...
    const { orderBy, limit, offset } = pagination ? pagination : ({} as any);
    ...
    if (userQuery.extra)
      query = this.db.from(
        this.db.raw(
          `(${userQuery.extra({ query, db: this }).toString()}) as tbl`
        )
      );
  ...
  countTableRecords({
    ...
    if (countOnly && countOnlyLimit) {
      ...
      query = this.db.from(this.db.raw(`(${query.toString()}) as limited`));
    }
